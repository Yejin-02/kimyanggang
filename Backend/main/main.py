...
def generate_word(category: str, difficulty: str) -> str:
    try:
        prompt = f"스무고개 게임을 하려고 하는데, '{category}' 중에서 난이도 '{difficulty}'으로 단어 예시를 하나 들어줘. 단답식으로만 대답해줘."
...
async def ask_question(question: str = Query(..., title="Question", description="Enter your question")):
    ...
    try:
        prompt = f"'{game_state.category}'중 하나인 '{game_state.answer}'에 대한 질문: '{question}' 예 아니오로만 대답해줘."
